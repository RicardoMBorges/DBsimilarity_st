...
def resolve_smiles_col(df: pd.DataFrame, wanted: str = "SMILES") -> Optional[str]:
    # try exact -> case-insensitive -> common aliases
    lowmap = {c.lower(): c for c in df.columns}
    if wanted in df.columns: 
        return wanted
    if wanted.lower() in lowmap:
        return lowmap[wanted.lower()]
    for alias in ["smiles", "canonicalsmiles", "canonical_smiles", "compound_smiles", "mol_smiles"]:
        if alias in lowmap:
            return lowmap[alias]
    return None
...
if run_descriptors:
    ...
    **`random_state`** controls the *random initialization* of points in the t-SNE algorithm.
    ...
    dendro_thresh = st.sidebar.number_input("**Dendrogram color threshold**", 1, 1_000_000, 25_000, 1_000)
...
if up_targets:
    for f in up_targets:
        if f.type == "chemical/x-mdl-sdfile" or f.name.lower().endswith(".sdf"):
            _df = read_sdf_to_df(f.read(), f.name)
        else:
            _df = smart_read_table(f.read(), f.name)
        _df["SourceFile"] = f.name  # <-- keep provenance
        frames_target.append(_df)
...
def _clean_enrich_frames(frames: List[pd.DataFrame], smiles_col: str) -> List[pd.DataFrame]:
    ...
    for df in frames:
        ...
        # resolve per-frame SMILES column (accepts canonicalsmiles, smiles, etc.)
        sm_col = resolve_smiles_col(d, smiles_col)

        if sm_col:
            d[sm_col] = d[sm_col].replace('', np.nan)
            d = d.dropna(subset=[sm_col]).drop_duplicates(subset=[sm_col])
            d = enrich_chem(d, sm_col)

            # normalize the name so the rest of the app can rely on `col_smiles`
            if sm_col != smiles_col:
                d = d.rename(columns={sm_col: smiles_col})
...
if not df_target.empty and sm_col:
    df0 = df_target.copy()
    df0[sm_col] = df0[sm_col].replace('', np.nan)
    df0 = df0.dropna(subset=[sm_col]).drop_duplicates(subset=[sm_col])
    df_target = enrich_chem(df0, sm_col)
...
# Stopwords set (single place)
try:
    from wordcloud import STOPWORDS as _WC_STOP, WordCloud  # type: ignore
    has_wc = True
    base_stopwords = set(_WC_STOP)
except Exception as e:
    has_wc = False
    base_stopwords = {
        "a","an","and","or","the","of","to","in","on","for","by","with","at","as",
        "from","that","this","these","those","is","are","be","was","were","it",
        "its","into","via","per","over","under","about"
    }
    st.info("Package `wordcloud` not found — showing bar chart fallback. "
            "Add `wordcloud` to requirements.txt (and redeploy) or `pip install wordcloud` locally.")


st.markdown("---")
st.subheader("Check Information from the Co-occurrent Compounds")
st.markdown("""
If any uploaded CSV has descriptive columns (e.g., *activities*, *targets*, *sources*),
select the column below to visualize a **wordcloud**. This helps spot frequent terms at a glance.
""")

with st.expander("Wordcloud / Term frequencies (from co-occurring or merged)"):
    # Pick source: co-occurring first (if available), else merged
    src_options = []
    if isinstance(co_df, pd.DataFrame) and not co_df.empty:
        src_options.append("Co-occurring (present in ALL input tables)")
    if isinstance(merged_df, pd.DataFrame) and not merged_df.empty:
        src_options.append("Merged (InchiKey-based)")

    if not src_options:
        st.info("No data available yet. Upload and merge tables first.")
    else:
        src_choice = st.radio(
            "Source table",
            src_options,
            index=0,
            horizontal=False,
            help="Use co-occurring rows to focus only on compounds shared across all inputs."
        )
        source_table = co_df if src_choice.startswith("Co-occurring") else merged_df

        # Candidate text columns (object dtype and not all-null)
        obj_cols = [c for c in source_table.columns
                    if source_table[c].dtype == "object" and source_table[c].notna().any()]
        if not obj_cols:
            st.info("No suitable text-like columns found in the chosen table.")
        else:
            col_text = st.selectbox(
                "Text column to analyze (e.g., activities, notes, class, origin)",
                obj_cols
            )

            c0, c1, c2, c3 = st.columns(4)
            with c0:
                max_rows_wc = st.number_input("Max rows scanned", 100, 1_000_000, 20_000, 500)
            with c1:
                min_len = st.number_input("Min word length", 1, 10, 3, 1)
            with c2:
                min_freq = st.number_input("Min frequency to keep", 1, 100, 2, 1)
            with c3:
                top_k = st.number_input("Max words kept", 20, 500, 200, 10)

            # Stopwords
            default_extra_sw = "activity,activities,compound,compounds,extract,extracts"
            extra_sw_text = st.text_area(
                "Extra stopwords (comma-separated)",
                value=default_extra_sw,
                help="Removed in addition to standard English stopwords."
            )

            normalize_accents = st.checkbox("Normalize accents (á→a, ç→c)", value=False,
                                            help="Helpful if many accented variants appear as separate words.")
            to_lower = st.checkbox("Lowercase terms", value=True)
            dedup_collocations = st.checkbox("Disable collocations (wordcloud bigrams)", value=True)

            # --------- FAST & CACHED TERM FREQUENCY ---------
            import re as _re
            SPLIT_RE = r"[;,/\|\s\(\)\[\]\{\}\:\.\+\-\_]+"  # flexible splitter

            @st.cache_data(show_spinner=False, max_entries=8)
            def _termfreq_cached(series_hash: int,
                                 min_len: int, min_freq: int, top_k: int,
                                 extra_sw: str, to_lower: bool, normalize_accents: bool) -> pd.DataFrame:
                # Rebuild the series from hash-bound stash (passed via session_state)
                s = st.session_state["_wc_series_stash"][series_hash].dropna().astype(str)

                if to_lower:
                    s = s.str.lower()
                if normalize_accents:
                    # lightweight accent removal without extra deps
                    s = s.apply(lambda x: x.encode("ascii", "ignore").decode("ascii"))

                # Split → explode → strip (vectorized)
                parts = s.str.split(SPLIT_RE, regex=True)
                tokens = parts.explode().dropna()
                tokens = tokens.str.strip()
                # Filter by length & digits
                tokens = tokens[tokens.str.len() >= int(min_len)]
                tokens = tokens[~tokens.str.fullmatch(r"\d+")]
                # Stopwords
                stopwords = set(base_stopwords)
                if extra_sw.strip():
                    stopwords.update([w.strip().lower() for w in extra_sw.split(",") if w.strip()])
                if stopwords:
                    tokens = tokens[~tokens.isin(stopwords)]

                # Count (very fast)
                freq = tokens.value_counts(dropna=False)
                # Apply min frequency & top-k
                freq = freq[freq >= int(min_freq)].head(int(top_k))
                return freq.rename_axis("term").reset_index(name="frequency")

            # Prepare the series (optionally limited) and a stable hash key
            raw_series = source_table[col_text].head(int(max_rows_wc))
            # Build a compact, stable content hash (sum of element-wise hashes)
            try:
                _hash = int(pd.util.hash_pandas_object(raw_series, index=False).sum())
            except Exception:
                # very unlikely, but safe fallback
                _hash = hash(tuple(raw_series.fillna("").tolist()))

            # Stash the original series for the cache to re-use without copying big data through the key
            if "_wc_series_stash" not in st.session_state:
                st.session_state["_wc_series_stash"] = {}
            st.session_state["_wc_series_stash"][_hash] = raw_series

            # Compute (cached)
            freq_df = _termfreq_cached(
                _hash, int(min_len), int(min_freq), int(top_k),
                extra_sw_text, bool(to_lower), bool(normalize_accents)
            )

            if freq_df.empty:
                st.info("No tokens left after filtering. Try lowering the minimums or removing stopwords.")
            else:
                show_df(freq_df.head(5), caption="Top terms (after filtering)")
                # Downloads
                freq_csv = freq_df.to_csv(index=False).encode("utf-8")
                st.download_button("Download term frequencies (CSV)", freq_csv, "term_frequencies.csv", "text/csv")

                # ---------- Render (optional) ----------
                render_wc = st.checkbox("Render wordcloud (optional)", value=False,
                                        help="Keeps UI snappy by avoiding heavy image generation unless requested.")
                if render_wc and has_wc:
                    try:
                        wc = WordCloud(
                            width=900, height=450, background_color="white",
                            stopwords=set(),  # already removed
                            collocations=not dedup_collocations,
                            max_words=int(top_k)
                        ).generate_from_frequencies(dict(zip(freq_df["term"], freq_df["frequency"])))
                        fig_wc, ax_wc = plt.subplots(figsize=(9, 4.5), dpi=150)
                        ...
                        plt.close(fig_wc)
                        buf.seek(0)
                        st.download_button("Download wordcloud (PNG)", buf, "wordcloud.png", "image/png")
                    except Exception as e:
                        st.warning(f"Wordcloud rendering failed, showing bar chart instead. ({e})")
                        has_wc = False

                if (not has_wc) or (render_wc and not has_wc) or (not render_wc):
                    # Fast fallback (and also the default visualization)
                    fig_bar = px.bar(freq_df.head(int(top_k)), x="term", y="frequency", title="Top terms (frequency)")
                    fig_bar.update_layout(xaxis_tickangle=-45, margin=dict(l=10, r=10, t=40, b=80))
                    st.plotly_chart(fig_bar, use_container_width=True)
                    st.download_button(
                        "Download bar plot (HTML)",
                        pio.to_html(fig_bar, include_plotlyjs='cdn', full_html=True).encode("utf-8"),
                        "term_frequency_bar.html",
                        "text/html"
                    )

                st.caption(
                    "Tip: If your activities are separated by semicolons/commas, the parser already splits on those. "
                    "Add domain-specific stopwords above (e.g., 'antibacterial,antioxidant')."
                )
...
st.markdown("---")
st.subheader("Filter: compounds similar to the targeted set (in_df1 == 1)")

similar_to_targeted_df = pd.DataFrame()

merged_df = st.session_state.get("merged_df", pd.DataFrame())
if merged_df.empty:
    st.info("Merged table not available yet.")
else:
    # Find the in_df* columns and enforce that in_df1 exists
    import re as _re
    in_cols = [c for c in merged_df.columns if _re.match(r"^in_df\d+$", c)]
    if "in_df1" not in in_cols:
        st.warning("Could not find 'in_df1' in the merged table. Upload targets and annotated to build flags.")
    else:
        sm_col_m = resolve_smiles_col(merged_df, col_smiles)
        if not sm_col_m:
            st.error("SMILES column not found in merged table.")
        else:
            # Targeted set A: rows where in_df1 == 1
            A = merged_df[(merged_df["in_df1"] == 1) & merged_df[sm_col_m].notna()].copy()

            # Comparison set B: rows where in_df1 == 0
            # Optionally require presence in at least one other input (any in_df2..n == 1)
            other_in_cols = [c for c in in_cols if c != "in_df1"]
            if other_in_cols:
                B_mask = (merged_df["in_df1"] == 0) & (merged_df[other_in_cols] == 1).any(axis=1)
            else:
                # If no other flags exist, allow everything not in_df1
                B_mask = (merged_df["in_df1"] == 0)
            B = merged_df[B_mask & merged_df[sm_col_m].notna()].copy()

            if A.empty:
                st.info("No targeted rows (in_df1 == 1) with valid SMILES.")
            elif B.empty:
                st.info("No non-target rows (in_df1 == 0) to compare.")
            else:
                # Controls
                c1, c2 = st.columns(2)
                with c1:
                    sim_thresh_first = st.slider(
                        "Similarity threshold to targeted set (in_df1)",
                        0.0, 1.0, 0.85, 0.01, key="sim_thresh_to_targeted"
                    )
                with c2:
                    metric_first = st.selectbox(
                        "Similarity metric", ["dice", "tanimoto"],
                        index=(0 if sim_metric == "dice" else 1),
                        key="metric_to_targeted"
                    )

                # Build FPs (Morgan r=2 / 2048 bits; same as the app)
                A_smiles = A[sm_col_m].astype(str).tolist()
                B_smiles = B[sm_col_m].astype(str).tolist()
                A_ids    = A.get("InchiKey", pd.Series([f"A_{i+1}" for i in range(len(A_smiles))])).astype(str).tolist()
                B_ids    = B.get("InchiKey", pd.Series([f"B_{i+1}" for i in range(len(B_smiles))])).astype(str).tolist()

                A_fps = fingerprints(A_smiles, radius=2, nBits=2048)
                B_fps = fingerprints(B_smiles, radius=2, nBits=2048)

                # Best match in A for each row in B
                best_sim, best_id = [], []
                for fp_b in B_fps:
                    if fp_b is None or not any(A_fps):
                        best_sim.append(0.0); best_id.append(None); continue
                    sims = (DataStructs.BulkDiceSimilarity(fp_b, A_fps)
                            if metric_first == "dice" else DataStructs.BulkTanimotoSimilarity(fp_b, A_fps))
                    if len(sims):
                        j = int(np.argmax(sims))
                        best_sim.append(float(sims[j]))
                        best_id.append(A_ids[j] if j < len(A_ids) else None)
                    else:
                        best_sim.append(0.0); best_id.append(None)

                B["MaxSim_to_Targeted(in_df1)"] = best_sim
                B["BestMatch_InchiKey_Targeted"] = best_id

                # Keep only B rows meeting the threshold
                similar_to_targeted_df = B[B["MaxSim_to_Targeted(in_df1)"] >= float(sim_thresh_first)] \
                                            .sort_values("MaxSim_to_Targeted(in_df1)", ascending=False)

                # Preview + download
                with st.expander("Preview similar-to-targeted rows"):
                    nrows = st.selectbox("Rows to show", [5, 10, 20, 50, 100], index=1, key="preview_sim_to_targeted")
                    show_df(similar_to_targeted_df.head(nrows),
                            caption=f"Similar to targeted (≥ {sim_thresh_first:.2f}) — top {nrows}")
                    st.caption(f"Kept {len(similar_to_targeted_df)} of {len(B)} non-target compounds.")

                if not similar_to_targeted_df.empty:
                    blob = similar_to_targeted_df.to_csv(sep=";", index=False).encode("utf-8")
                    st.download_button(
                        "Download similar_to_targeted.csv",
                        data=blob,
                        file_name="similar_to_targeted.csv",
                        key="dl_similar_to_targeted_csv"
                    )
                    exports["similar_to_targeted.csv"] = blob
        ...
        if n_samples < 2 or n_features < 1:
            ...
            with st.expander("Dendrogram controls (compute clusters)"):
                ...
                dist_metric = st.selectbox(
                    "Distance metric",
                    ["euclidean","cityblock","cosine","correlation","chebyshev","canberra","braycurtis","minkowski"],
                    index=0
                )

                # Ward requires Euclidean and uses the observation matrix X directly
                use_X_direct = (linkage_method == "ward")
                if use_X_direct:
                    if dist_metric != "euclidean":
                        st.info("Linkage 'ward' requires Euclidean; overriding metric to 'euclidean'.")
                    dist_metric = "euclidean"
                    Z = linkage(X, method="ward", metric="euclidean")  # uses X (not condensed D)
                else:
                    D = pdist(X, metric=dist_metric)
                    Z = linkage(D, method=linkage_method)

                # Bounds from the actual merge heights
                z_heights = Z[:, 2]
                zmin, zmax = float(np.min(z_heights)), float(np.max(z_heights))
                thr = float(np.clip(dendro_thresh, zmin, zmax))  # respect sidebar "Dendrogram color threshold"

                st.caption(f"Color cut = {thr:.3g}  |  valid range [{zmin:.3g}, {zmax:.3g}]")

                # Persist for the viewer and t-SNE coloring
                st.session_state["_dendro_Z"] = Z
                st.session_state["_dendro_thr"] = thr

                labels_ = fcluster(Z, t=thr, criterion="distance")
                st.session_state["_cluster_labels"] = labels_
                st.caption(f"Formed {len(np.unique(labels_))} clusters at cut = {thr:.3g}.")

            # ----------------- Dendrogram (view / save) -----------------
            with st.expander("Dendrogram (view / save)"):
                Z_use = st.session_state.get("_dendro_Z")
                thr_use = st.session_state.get("_dendro_thr")

                if Z_use is None or thr_use is None:
                    st.info("Adjust the sidebar threshold once to define the cut.")
                else:
                    # Viewer size controls (persistent)
                    c1, c2, c3 = st.columns(3)
                    with c1:
                        st.session_state["_dend_fig_w"] = st.number_input(
                            "Width (inches)", 6.0, 20.0, float(st.session_state.get("_dend_fig_w", 10.0)), 0.5
                        )
                    with c2:
                        st.session_state["_dend_fig_h"] = st.number_input(
                            "Height (inches)", 3.0, 20.0, float(st.session_state.get("_dend_fig_h", 6.0)), 0.5
                        )
                    with c3:
                        st.session_state["_dend_dpi"] = st.number_input(
                            "DPI (PNG only)", 100, 600, int(st.session_state.get("_dend_dpi", 200)), 25
                        )
                    fig_w = float(st.session_state["_dend_fig_w"])
                    fig_h = float(st.session_state["_dend_fig_h"])
                    dpi   = int(st.session_state["_dend_dpi"])

                    # Labels (fallback if InchiKey missing)
                    ids_re = df_target.get(
                        "InchiKey",
                        pd.Series([f"mol_{i+1}" for i in range(X.shape[0])])
                    ).astype(str).tolist()
                    ...
                    ax.set_ylabel("Distance"); ax.set_xlabel("Samples")
